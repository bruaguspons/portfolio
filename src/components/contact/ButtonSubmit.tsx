...
const ButtonSubmit = ({children}:Props) => {
  return (
    <button
            ...
            type="submit"
        >
            <span className="relative px-5 py-2.5 transition-all ease-in-out duration-150 bg-background-100 dark:bg-background-600 rounded-md w-max sm:group-hover:bg-transparent group-active:bg-transparent">
                {children}
            </span>
        </button>
  )
