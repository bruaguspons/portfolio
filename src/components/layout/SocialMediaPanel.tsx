...
import CloseSocialMediaPanel from "./CloseSocialMediaPanel";
...
export default function SocialMediaPanel(): React.ReactNode {
  ...
  useEffect(() => {
    setTimeout(() => setToken(randomString()), 0);

    const sub = greyBgSubjectManager.getSubject().subscribe(setShowPanel);
    ...
    >
      ...
      <div className="flex h-full w-full flex-col items-center justify-evenly">
        ...
        <GradientIcon
          ...
          desc="CV"
          ariaLabel="CV button"
          href={`/CV_BRUNO_PONS_ES.pdf?${token}`}
          target="_blank"
          className="flex w-20"
          viewBox="0 0 24 24"
          fill="none"
